...
final class SourceChartsTest extends CIUnitTestCase
{
    ...
    public function testUploadChartsBatchStoresMultipleChartsIndependently(): void
    {
        [$sourceIdA] = $this->createSourceWithEpochs(3);
        [$sourceIdB] = $this->createSourceWithEpochs(2);

        $result = $this->withHeaders($this->authHeaders())
            ->withBodyFormat('json')
            ->post('/api/v1/sources/charts/batch', [
                'charts' => [
                    ['source_id' => $sourceIdA, 'style' => 'track', 'frame_count' => 3, 'png_base64' => base64_encode(self::MINIMAL_PNG)],
                    ['source_id' => $sourceIdB, 'style' => 'stamp_strip', 'frame_count' => 2, 'png_base64' => base64_encode(self::MINIMAL_PNG)],
                ],
            ]);
        ...
        $this->assertSame($sourceIdA, $json['results'][0]['source_id']);
        $this->assertSame('track', $json['results'][0]['style']);
        $this->assertSame('ok', $json['results'][1]['status']);
        $this->assertSame($sourceIdB, $json['results'][1]['source_id']);

        $this->assertFileExists(WRITEPATH . 'uploads/charts/' . $sourceIdA . '.png');
        $this->assertFileExists(WRITEPATH . 'uploads/charts/' . $sourceIdB . '.png');

        $db = \Config\Database::connect('default');
        $this->assertSame(1, $db->table('source_charts')->where('source_id', $sourceIdA)->countAllResults());
        $this->assertSame(1, $db->table('source_charts')->where('source_id', $sourceIdB)->countAllResults());

        @unlink(WRITEPATH . 'uploads/charts/' . $sourceIdA . '.png');
        @unlink(WRITEPATH . 'uploads/charts/' . $sourceIdB . '.png');
    }

    public function testUploadChartsBatchPartialFailureDoesNotBlockOtherEntries(): void
    {
        [$sourceIdA] = $this->createSourceWithEpochs(2);
        ...
        $result = $this->withHeaders($this->authHeaders())
            ...
            ->post('/api/v1/sources/charts/batch', [
                'charts' => [
                    ['source_id' => $sourceIdA, 'style' => 'bogus-style', 'frame_count' => 2, 'png_base64' => base64_encode(self::MINIMAL_PNG)],
                    ['source_id' => 'unknownsource.00000000', 'style' => 'track', 'frame_count' => 1, 'png_base64' => base64_encode(self::MINIMAL_PNG)],
                ],
            ]);
        ...
        $this->assertSame($sourceIdA, $json['results'][0]['source_id']);
        $this->assertSame('error', $json['results'][1]['status']);
        $this->assertSame('Source not found', $json['results'][1]['error']);

        $this->assertFileDoesNotExist(WRITEPATH . 'uploads/charts/' . $sourceIdA . '.png');
    }

    public function testUploadChartsBatchRejectsMissingChartsField(): void
    {
        $result = $this->withHeaders($this->authHeaders())
            ->withBodyFormat('json')
            ->post('/api/v1/sources/charts/batch', ['oops' => []]);

        $result->assertStatus(400);
    }

    public function testUploadChartsBatchWithEmptyChartsArrayReturnsEmptyResults(): void
    {
        $result = $this->withHeaders($this->authHeaders())
            ->withBodyFormat('json')
            ->post('/api/v1/sources/charts/batch', ['charts' => []]);
